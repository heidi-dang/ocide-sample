...
###   ocidecore.org and ocide.org and compares them.
...
WORKINGDIR="/tmp/ocide_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://ocidecore.org"
HOST2="https://ocide.org"
BASEDIR="/bin/"
VERSIONPREFIX="ocide-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'ocide-',
...
if [ $? -ne 0 ]; then
   echo "ocide.org failed to provide signature file, but ocidecore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "ocide.org and ocidecore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the OCIDE Core binary release signing key installed?"
   fi
