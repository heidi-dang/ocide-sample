...
The RPM spec file provided here is for OCIDE-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://ocide.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[OCIDE git repository](https://github.com/ocide/ocide) but are not part of
...
Currently only `Source10` is used, for the example `ocide.conf` file.
...
in the ocide git repository. At present only an SVG version of the OCIDE
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the ocide
...
needed when 0.12.1 is released, a proper fix is already in the OCIDE git
master branch.
...
idea to build OCIDE Core against a static version of that library compiled
...
The `ocide.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `ocide-server` package. At this point, most RPM based
...
The `ocide-server` package should have SELinux support. How to properly do
...
OCIDE-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/ocide/). He has been
packaging OCIDE for Fedora at least since 2012.
...
4. I renamed the `ocide` package that contains the Qt GUI to `ocide-core` as
that appears to be how the general population refers to it, in contrast to
`ocide-xt` or `ocide-classic`. I wanted to make sure the general population
