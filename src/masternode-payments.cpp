...
*
*   Why is this needed?
*   - In OCide some blocks are superblocks, which output much higher amounts of coins
...
bool CMasternodeBlockPayees::IsTransactionValid(const CTransactionRef& txNew) const
{
    ...
    LogPrintf("CMasternodeBlockPayees::IsTransactionValid -- ERROR: Missing required payment, possible payees: '%s', amount: %f OCIDE\n", strPayeesPossible, (float)nMasternodePayment/COIN);
    return false;
}
...
void AdjustMasternodePayment(CMutableTransaction &tx, const CTxOut &txoutMasternodePayment, const SPoSContract &sposContract)
{
    ...
    if(it != std::end(tx.vout))
    {
        ...
        if(sposContract.IsValid()) // here we have 3 outputs, first as stake reward, second as spos reward, third as MN reward
        {
            masternodePayment /= 100; // to calculate percentage
            tx.vout[i - 1].nValue -= masternodePayment * sposContract.stakePercentage; // adjust reward for owner.
            tx.vout[i].nValue -= masternodePayment * (100 - sposContract.stakePercentage); // adjust reward for secure
        }
