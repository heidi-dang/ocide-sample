...
bool PaymentRequestPlus::getSecure(X509_STORE* certStore, QString& secure) const
{
    secure.clear();
    ...
    if (paymentRequest.pki_type() == "x509+sha256") {
        ...
        qWarning() << "PaymentRequestPlus::getSecure: Payment request: pki_type == none";
        return false;
    }
    else {
        qWarning() << "PaymentRequestPlus::getSecure: Payment request: unknown pki_type " << QString::fromStdString(paymentRequest.pki_type());
        return false;
    }

    payments::X509Certificates certChain;
    if (!certChain.ParseFromString(paymentRequest.pki_data())) {
        qWarning() << "PaymentRequestPlus::getSecure: Payment request: error parsing pki_data";
        return false;
    }
    ...
    for (int i = 0; i < certChain.certificate_size(); i++) {
        ...
        if (currentTime < qCert.effectiveDate() || currentTime > qCert.expiryDate()) {
            qWarning() << "PaymentRequestPlus::getSecure: Payment request: certificate expired or not yet active: " << qCert;
            return false;
        }
#if QT_VERSION >= 0x050000
        if (qCert.isBlacklisted()) {
            qWarning() << "PaymentRequestPlus::getSecure: Payment request: certificate blacklisted: " << qCert;
            return false;
        }
#endif
    ...
    if (certs.empty()) {
        qWarning() << "PaymentRequestPlus::getSecure: Payment request: empty certificate chain";
        return false;
    }
    ...
    if (!store_ctx) {
        qWarning() << "PaymentRequestPlus::getSecure: Payment request: error creating X509_STORE_CTX";
        return false;
    }
    ...
    try
    {
        ...
        if (result != 1) {
            ...
            if (!(error == X509_V_ERR_DEPTH_ZERO_SELF_SIGNED_CERT && gArgs.GetBoolArg("-allowselfsignedrootcertificates", DEFAULT_SELFSIGNED_ROOTCERTS))) {
               ...
               qDebug() << "PaymentRequestPlus::getSecure: Allowing self signed root certificate, because -allowselfsignedrootcertificates is true.";
            }
        }
        ...
        // TODO: detect EV certificates and set secure = business name instead of unfriendly NID_commonName ?
    }
    catch (const SSLVerifyError& err) {
        fResult = false;
        qWarning() << "PaymentRequestPlus::getSecure: SSL error: " << err.what();
    }
