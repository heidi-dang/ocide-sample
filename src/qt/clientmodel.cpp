...
QString ClientModel::getSecurenodeCountString() const
{
    // return tr("Total: %1 (PS compatible: %2 / Enabled: %3) (IPv4: %4, IPv6: %5, TOR: %6)").arg(QString::number((int)securenodeman.size()))
    return tr("Total: %1 (PS compatible: %2 / Enabled: %3)")
                .arg(QString::number((int)m_node.getNumSecurenodes().size))
                .arg(QString::number((int)m_node.getNumSecurenodes().countEnabledProtVersion))
                .arg(QString::number((int)m_node.getNumSecurenodes().countEnabled));
            // .arg(QString::number((int)securenodeman.CountByIP(NET_IPV4)))
            // .arg(QString::number((int)securenodeman.CountByIP(NET_IPV6)))
    // .arg(QString::number((int)securenodeman.CountByIP(NET_TOR)));
}
...
void ClientModel::updateMnTimer()
{
    ...
    QString newSecurenodeCountString = getSecurenodeCountString();
    ...
    if (cachedSecurenodeCountString != newSecurenodeCountString)
    {
        cachedSecurenodeCountString = newSecurenodeCountString;

        Q_EMIT strSecurenodesChanged(cachedSecurenodeCountString);
    }
}
