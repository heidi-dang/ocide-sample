...
const QString BITCOIN_IPC_PREFIX("ocide:");
...
void PaymentServer::ipcParseCommandLine(interfaces::Node& node, int argc, char* argv[])
{
    for (int i = 1; i < argc; i++)
    {
        ...
        // If the ocide: URI contains a payment request, we are not able to detect the
        // network as that would require fetching and parsing the payment request.
        // That means clicking such an URI which contains a testnet payment request
        // will start a mainnet instance and throw a "wrong network" error.
        if (arg.startsWith(BITCOIN_IPC_PREFIX, Qt::CaseInsensitive)) // ocide: URI
        {
    ...
    optionsModel(0)
{
    ...
    // on Mac: sent when you click ocide: links
    ...
    if (startLocalServer)
    {
        ...
        if (!uriServer->listen(name)) {
            ...
            QMessageBox::critical(0, tr("Payment request error"),
                tr("Cannot start ocide: click-to-pay handler"));
        }
        else {
...
//
// OSX-specific way of handling ocide: URIs and PaymentRequest mime types.
...
void PaymentServer::initNetManager()
{
    ...
    // netManager is used to fetch paymentrequests given in ocide: URIs
...
void PaymentServer::handleURIOrFile(const QString& s)
{
    ...
    if (s.startsWith("ocide://", Qt::CaseInsensitive))
    {
        Q_EMIT message(tr("URI handling"), tr("'ocide://' is not a valid URI. Use 'ocide:' instead."),
            CClientUIInterface::MSG_ERROR);
    }
    else if (s.startsWith(BITCOIN_IPC_PREFIX, Qt::CaseInsensitive)) // ocide: URI
    {
#if QT_VERSION < 0x050000
        QUrl uri(s);
#else
        ...
        else // normal URI
        {
            ...
            if (GUIUtil::parseBitcoinURI(s, &recipient))
            {
                ...
                Q_EMIT message(tr("URI handling"),
                    tr("URI cannot be parsed! This can be caused by an invalid OCIDE address or malformed URI parameters."),
                    CClientUIInterface::ICON_WARNING);
...
bool PaymentServer::processPaymentRequest(const PaymentRequestPlus& request, SendCoinsRecipient& recipient)
{
    ...
    request.getSecure(certStore.get(), recipient.authenticatedSecure);
    ...
    for (const std::pair<CScript, CAmount>& sendingTo : sendingTos) {
        ...
        else if (!recipient.authenticatedSecure.isEmpty()) {
    ...
    if (!recipient.authenticatedSecure.isEmpty()) {
        qDebug() << "PaymentServer::processPaymentRequest: Secure payment request from " << recipient.authenticatedSecure;
    }
    else {
...
void PaymentServer::fetchPaymentACK(WalletModel* walletModel, const SendCoinsRecipient& recipient, QByteArray transaction)
{
    ...
    payment.set_secure_data(details.secure_data());
    ...
    if (walletModel->wallet().getKeyFromPool(false /* internal */, newKey)) {
        ...
        std::string label = tr("Refund from %1").arg(recipient.authenticatedSecure).toStdString();
