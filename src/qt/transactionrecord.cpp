...
QList<TransactionRecord> TransactionRecord::decomposeTransaction(const interfaces::WalletTx& wtx, interfaces::Wallet& wallet)
{
    ...
    if(wtx.tx->IsCoinStake())
    {
        ...
        if (!wtx.txout_is_mine[1])
        {
            ...
            // this might be masternode reward, or spos block reward.

            // if last output was ours, it means that it's spos reward
            CAmount stakeAmount = 0;
            CAmount commissionAmount = 0;
            CTxDestination sposAddress;
            CTxDestination secureAddress;
            if(wallet.getSPoSPayments(wtx.tx, stakeAmount, commissionAmount, sposAddress, secureAddress))
            {
                //stake reward
                sub.involvesWatchAddress = false;
                sub.type = TransactionRecord::StakeMintSPoS;
                sub.address = EncodeDestination(sposAddress);
        ...
        else
        {
            ...
            CTxDestination sposAddress;
            CTxDestination secureAddress;
            bool isSPoSBlock = wallet.getSPoSPayments(wtx.tx, stakeAmount, commissionAmount, sposAddress, secureAddress);
            ...
            if(isSPoSBlock)
            {
                ...
                {
                    sub.type = TransactionRecord::StakeMintSPoSCommission;
                    sub.address = EncodeDestination(secureAddress);
