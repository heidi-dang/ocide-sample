...
void SendCoinsDialog::on_sendButton_clicked()
{
    ...
    for (const SendCoinsRecipient &rcp : currentTransaction.getRecipients())
    {
        ...
        else if(!rcp.authenticatedSecure.isEmpty()) // authenticated payment request
        {
            recipientElement.append(tr("%1 to %2").arg(amount, GUIUtil::HtmlEscape(rcp.authenticatedSecure)));
        }
...
void SendCoinsDialog::coinControlChangeEdited(const QString& text)
{
    if (model && model->getAddressTableModel())
    {
        ...
        {
            ui->labelCoinControlChangeLabel->setText(tr("Warning: Invalid OCIDE address"));
        }
        else // Valid address
        {
