...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a OCIDE address (e.g. %1)").arg(
...
{
    // return if URI is not valid or is no ocide: URI
    if(!uri.isValid() || uri.scheme() != QString("ocide"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if(!BitcoinUnits::parse(BitcoinUnits::OCIDE, i->second, &rv.amount))
                {
                    return false;
                }
            }
...
{
    QString ret = QString("ocide:%1").arg(info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::OCIDE, info.amount, false, BitcoinUnits::separatorNever));
        paramCount++;
    }
...
void restoreWindowGeometry(const QString &strSetting, const QSize &defaultSize, QWidget *parent)
{
    ...
    if ((!pos.x() && !pos.y()) || (QApplication::desktop()->screenNumber(parent) == -1))
    {
        ...
        QPoint defaulsPos = screen.center() -
                QPoint(defaultSize.width() / 2, defaultSize.height() / 2);
        parent->resize(defaultSize);
        parent->move(defaulsPos);
    }
}
...
void openConfigfile()
{
    ...
    /* Open ocide.conf with the associated application */
