...
void PaymentServerTests::paymentServerTests()
{
    ...
    r.paymentRequest.getSecure(caStore, secure);
    QCOMPARE(secure, QString("testsecure.org"));

    // Signed, but expired, secure cert in the request:
    data = DecodeBase64(paymentrequest2_cert1_BASE64);
    r = handleRequest(server, data);
    r.paymentRequest.getSecure(caStore, secure);
    QCOMPARE(secure, QString(""));

    // 10-long certificate chain, all intermediates valid:
    data = DecodeBase64(paymentrequest3_cert1_BASE64);
    r = handleRequest(server, data);
    r.paymentRequest.getSecure(caStore, secure);
    QCOMPARE(secure, QString("testsecure8.org"));
    ...
    // Try again with no root CA's, verifiedSecure should be empty:
