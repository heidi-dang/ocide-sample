...
{
    return QCoreApplication::translate("ocide-core", psz).toStdString();
}
...
void BitcoinApplication::initializeResult(bool success)
{
    ...
    if(success)
    {
        ...
        // ocide: URIs or payment requests:
...
{
    QMessageBox::critical(0, "Runaway exception", BitcoinGUI::tr("A fatal error occurred. OCIDE can no longer continue safely and will quit.") + QString("\n\n") + message);
    ::exit(EXIT_FAILURE);
}
...
int main(int argc, char *argv[])
{
    ...
    Q_INIT_RESOURCE(ocide);
    Q_INIT_RESOURCE(ocide_locale);
    ...
    if(!securenodeConfig.read(strErr)) {
        QMessageBox::critical(0, QObject::tr(PACKAGE_NAME),
                              QObject::tr("Error reading securenode configuration file: %1").arg(strErr.c_str()));
        return EXIT_FAILURE;
    }
    ...
    // ocide: links repeatedly have their payment requests routed to this process:
