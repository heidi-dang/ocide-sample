...
const char * const BITCOIN_CONF_FILENAME = "ocide.conf";
const char * const BITCOIN_PID_FILENAME = "ocided.pid";

//OCIDE only features
bool fMasterNode = false;
bool fLiteMode = false;
bool fSecureNode = false;
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager &am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "ocided -foo=bar
...
std::string ArgsManager::GetHelpMessage()
{
    ...
    for (const auto& arg_map : m_available_args) {
        ...
        case OptionsCategory::SECURENODE:
            usage += HelpMessageGroup("Securenode options:"); break;
        default:
            break;
        }
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\OCIDE
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\OCIDE
    // Mac: ~/Library/Application Support/OCIDE
    // Unix: ~/.ocide
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "OCIDECore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/OCIDECore";
#else
    // Unix
    return pathRet / ".ocidecore";
#endif
#endif
}
...
fs::path GetSecurenodeConfigFile()
{
    boost::filesystem::path pathConfigFile(gArgs.GetArg("-securenodeconf", "securenode.conf"));
    if (!pathConfigFile.is_complete())
        return fs::absolute(pathConfigFile, GetDataDir());
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    // Check for untranslated substitution to make sure OCIDE Core copyright is not removed by accident
    if (strprintf(COPYRIGHT_HOLDERS, COPYRIGHT_HOLDERS_SUBSTITUTION).find("OCIDE Core") == std::string::npos) {
