...
//test hash vs target
bool stakeTargetHit(uint256 hashProofOfStake, int64_t nValueIn, arith_uint256 bnTargetPerCoinDay, bool fSPoS)
{
    //get the stake weight - weight is equal to coin amount
    arith_uint256 bnCoinDayWeight = arith_uint256(nValueIn) / 100;

    //Spos weight penalty -- first try. This might need to be modified later depending on testing
    //    if(fSPoS){
...
bool CheckProofOfStake(const CBlock &block, uint256& hashProofOfStake)
{
    ...
    //verify signature and script, don't check script if it's spos block, signature check will happen in different place
