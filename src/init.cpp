...
#include <net_processing_ocide.h>
 ...
 * between ocided, and ocide-qt and non-server tools.
*/
...
static bool LoadExtensionsDataCaches()
{
    ...
    CFlatDB<CSecurenodeMan> flatdb5("securenodecache.dat", "magicSecurenodeCache");
    if(!flatdb5.Load(securenodeman)) {
        return InitError(_("Failed to load securenode cache from") + "\n" + (pathDB / strDBName).string());
    }
...
void SetupServerArgs()
{
    ...
    gArgs.AddArg("-securenode=<n>", "Enable the client to act as a securenode (0-1, default: false", false, OptionsCategory::SECURENODE);
    gArgs.AddArg("-securenodeprivkey=<n>", "Set the masternode private key", false, OptionsCategory::SECURENODE);
    gArgs.AddArg("-securenodeconf=<file>", "Specify securenode configuration file (default: securenode.conf)", false, OptionsCategory::SECURENODE);
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/ocide/ocide>";
    const std::string URL_WEBSITE = "<https://ocidecore.org>";
...
/** Sanity checks
 *  Ensure that OCIDE is running in a usable environment with all
        ...
        for (const std::string& strDeployment : gArgs.GetArgs("-vbparams")) {
            ...
            for (int j=0; j<(int)Consensus::MAX_VERSION_BITS_DEPLOYMENTS; ++j)
            {
                if (vDeploymentParams[0].compare(VersionBitsDeploymentInfo[j].name) == 0) {
                    UpdateVersionBitsParameters(Consensus::DeploymensPos(j), nStartTime, nTimeout);
                    found = true;
...
{
    // Make sure only a single OCIDE process is using the data directory.
...
bool AppInitPrivateSend()
{
    ...
    if(!securenodeConfig.read(strErr)) {
        LogPrintf("Error reading securenode configuration file: %s\n", strErr.c_str());
        return false;
    }

    fMasterNode = gArgs.GetBoolArg("-masternode", false);
    fSecureNode = gArgs.GetBoolArg("-securenode", false);
    ...
    if(fSecureNode) {
        LogPrintf("SECURENODE:\n");

        std::string strSecureNodePrivKey = gArgs.GetArg("-securenodeprivkey", "");
        if(!strSecureNodePrivKey.empty()) {
            if(!CMessageSigner::GetKeysFromSecret(strSecureNodePrivKey, activeSecurenode.keySecurenode, activeSecurenode.pubKeySecurenode))
                return InitError(_("Invalid securenodeprivkey. Please see documenation."));

            LogPrintf("  pubKeySecurenode: %s\n", activeSecurenode.pubKeySecurenode.GetID().ToString());
        } else {
            return InitError(_("You must specify a securenodeprivkey in the configuration. Please see documentation for help."));
        }
    }
...
bool AppInitMain()
{
    ...
    if (gArgs.IsArgSet("-datadir") && !fs::path(gArgs.GetArg("-datadir", "")).is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the " /* Continued */
                  "current working directory '%s'. This is fragile, because if ocide is started in the future "
                  "from a different location, it will be unable to locate the current data files. There could "
                  "also be data loss if ocide is started while in a temporary directory.\n",
    ...
    // ********************************************************* Step 11c: update block tip in OCIDE modules
    ...
    // ********************************************************* Step 11d: start thread for ocide extensions

    threadGroup.create_thread(boost::bind(net_processing_ocide::ThreadProcessExtensions, g_connman.get()));
