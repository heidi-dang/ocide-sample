...
void CNode::copyStats(CNodeStats &stats)
{
    ...
    // Raw ping time is in microseconds, but show it to user as whole seconds (OCIDE users should be well used to small numbers with many decimal places by now :)
...
static void ThreadMapPort()
{
    ...
    if (r == 1)
    {
        ...
        std::string strDesc = "OCIDE " + FormatFullVersion();
...
CNode *CConnman::OpenSecurenodeConnection(const CAddress &addrConnect)
{
    if(auto pNode = OpenNetworkConnectionImpl(addrConnect, true, nullptr, nullptr, false, false, false, true))
    {
        if(!pNode->fSecurenode)
