...
bool WalletBatch::WriteSPoSContractTx(uint256 hash, const CWalletTx &wtx)
{
    return WriteIC(std::make_pair(std::string("tpsctx"), hash), wtx);
}

bool WalletBatch::EraseSPoSContractTx(uint256 hash)
{
             ...
             CWalletScanState &wss, std::string& strType, std::string& strErr)
{
    try {
        ...
        else if (strType == "tpsctx")
        {
            ...
            pwallet->LoadSPoSContractFromDB(wtx);
