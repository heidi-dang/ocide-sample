...
{
    for(auto &&contractTx : std::move(sposContractsTxLoadedFromDB))
    {
        if(!LoadSPoSContract(contractTx))
        {
            // if contract was not added, there is a big chance that it's deprecated, let's cleanup watch only address
            if(SPoSUtils::IsSPoSSecureContract(this, contractTx.tx))
            {
                auto sposContract = SPoSContract::FromSPoSContractTx(contractTx.tx);
                if(sposContract.IsValid())
                {
                    auto script = GetScriptForDestination(sposContract.sposAddress.Get());
...
bool CWallet::CreateCoinStakeKernel(CScript &kernelScript, const CScript &stakeScript,
                                    ...
                                    const SPoSContract &contract, bool fGenerateSegwit, bool fPrintProofOfStake) const
{
...
void CWallet::FillCoinStakePayments(CMutableTransaction &transaction,
                                    const SPoSContract &sposContract,
                                    ...
                                    CAmount blockReward) const
{
    ...
    if(sposContract.IsValid())
        percentage = sposContract.stakePercentage; // first vout is owner


    // if this is spos, we need to age coin, without paying reward to this address.
    //                if(!sposContract.IsValid())
    ...
    // here we need to send reward to secure to his reward address
    if(sposContract.IsValid())
    {
        transaction.vout.emplace_back(GetStakeReward(blockReward, 100 - sposContract.stakePercentage),
                                      GetScriptForDestination(sposContract.secureAddress.Get()));
    }
}

bool CWallet::IsSPoSContractSpent(COutPoint outpoint) const
{
    SPoSContract contract;
    for(const auto &container : {sposOwnerContracts, sposSecureContracts})
    {
    ...
    return contract.IsValid() && SPoSUtils::GetContractCollateralOutpoint(contract) == outpoint;
}
...
bool CWallet::AddToWalletIfSPoSContract(const CTransactionRef &tx)
{
    AssertLockHeld(cs_wallet);

    if(SPoSUtils::IsSPoSContract(tx))
    {
        // TODO: FIND CORRECT CONDITIONS TO erase contract
        //        if(!mempool.exists(tx.GetHash()))
        //        {
        //            std::cout << "Tx doesn't exist in mempool\n");
        //            if(sposOwnerContracts.count(tx.GetHash()))
        //                sposOwnerContracts.erase(tx.GetHash());

        //            if(sposSecureContracts.count(tx.GetHash()))
        //                sposSecureContracts.erase(tx.GetHash());

        //            CWalletDB walletDb(strWalletFile);
        //            walletDb.EraseSPoSContractTx(tx.GetHash());

        //            return true;
        //        }
        //        else
        {
            bool isSecure = SPoSUtils::IsSPoSSecureContract(this, tx);
            bool isOwner = SPoSUtils::IsSPoSOwnerContract(this, tx);

            if(isSecure || isOwner)
            {
                auto contract = SPoSContract::FromSPoSContractTx(tx);

                CWalletTx wtx(this, tx);
                if(LoadSPoSContract(wtx))
                {
                    WalletBatch walletDb(*database);
                    walletDb.WriteSPoSContractTx(wtx.GetHash(), wtx);


                    if(isSecure && !isOwner) {
                        AddWatchOnly(GetScriptForDestination(contract.sposAddress.Get()));
                    }
                }
                else
                {
...
static bool IsCorrectType(CAmount nAmount, AvailableCoinsType nCoinType)
{
    ...
    } else if(nCoinType == ONLY_SECURENODE_COLLATERAL) {
    ...
    for (const auto& entry : mapWallet)
    {
        ...
        for (unsigned int i = 0; i < pcoin->tx->vout.size(); i++) {
            ...
            if (IsLockedCoin(entry.first, i) && nCoinType != ONLY_MASTERNODE_COLLATERAL &&
                    nCoinType != ONLY_SECURENODE_COLLATERAL)
                continue;
...
std::map<CTxDestination, std::vector<COutput>> CWallet::ListCoins() const
{
    ...
    // https://github.com/ocide/ocide/pull/10340, but that change has been
    // postponed until after https://github.com/ocide/ocide/pull/10244 to
...
bool CWallet::SelectStakeCoins(StakeCoinsSet &setCoins, CAmount nTargetAmount, bool fSelectWitness, const CScript &scriptFilterPubKey) const
{
    ...
    for (const COutput& out : vCoins) {
        ...
        if(rejectCache.count(scriptPubKeyCoin)) {
            ...
            auto it = std::find_if(std::begin(sposOwnerContracts), std::end(sposOwnerContracts), [scriptPubKeyCoin](const std::pair<uint256, SPoSContract> &entry) {
                return GetScriptForDestination(entry.second.sposAddress.Get()) == scriptPubKeyCoin;
            });

            if(it != std::end(sposOwnerContracts)) {
        ...
        nAmountSelected += out.tx->tx->vout[out.i].nValue; //maybe change here for spos
                                ...
                                int& nChangePosInOut, std::string& strFailReason, const CCoinControl& coin_control, bool sign, OnTransactionToBeSigned onTxToBeSigned)
{
    ...
    int nBytes;
    {
        ...
        LOCK2(cs_main, cs_wallet);
        {
            ...
            // change transaction isn't always pay-to-ocide-address
            CScript scriptChange;
                              ...
                              bool fGenerateSegwit)
{
    ...
    bool fIsSPoS = sposContract.IsValid();
    if (GetTime() - nLastStakeSetUpdate > nStakeSetUpdateTime) {
        ...
        if(fIsSPoS)
        {
            scriptPubKey = GetScriptForDestination(sposContract.sposAddress.Get());
            LogPrint(BCLog::KERNEL, "finding spos, contract sposAddress: %s\n", sposContract.sposAddress.ToString().c_str());
        }
    ...
    COutPoint sposContractOutpoint = SPoSUtils::GetContractCollateralOutpoint(sposContract);
    for(const std::pair<const CWalletTx*, unsigned int> &pcoin : setStakeCoins)
    {
        // this is probably collateral, don't stake it.
        if(pcoin.first->GetHash() == sposContractOutpoint.hash &&
                pcoin.second == sposContractOutpoint.n)
            continue;
        ...
        fKernelFound = CreateCoinStakeKernel(kernelScript, stakeScript, nBits,
                                             ...
                                             prevoutStake, nTxNewTime, sposContract, fGenerateSegwit, false);

        if(fKernelFound)
        {
            if(!fIsSPoS) // we won't sign in case of spos block
                vwtxPrev.push_back(pcoin.first);

            FillCoinStakePayments(txNew, sposContract, kernelScript, prevoutStake, blockReward);
            break;
        }
    }
    ...
    AdjustMasternodePayment(txNew, txoutMasternode, sposContract);
...
bool CWallet::LoadSPoSContract(const CWalletTx &walletTx)
{
    bool isSecure = SPoSUtils::IsSPoSSecureContract(this, walletTx.tx);
    bool isOwner = SPoSUtils::IsSPoSOwnerContract(this, walletTx.tx);

    if(!isSecure && !isOwner)
        return false; // shouldn't happen

    auto contract = SPoSContract::FromSPoSContractTx(walletTx.tx);
    ...
    if(isSecure) {
        sposSecureContracts[txHash] = contract;
    }
    else {
        // we need to lock this coin, because it can be spend
        sposOwnerContracts[txHash] = contract;
        LockCoin(SPoSUtils::GetContractCollateralOutpoint(contract));
    }

    return true;
}

void CWallet::LoadSPoSContractFromDB(CWalletTx walletTx)
{
    sposContractsTxLoadedFromDB.emplace_back(walletTx);
}

bool CWallet::RemoveSPoSContract(const uint256 &contractTxId)
{
    AssertLockHeld(cs_wallet);

    if (!WalletBatch(*database).EraseSPoSContractTx(contractTxId))
        return false;

    if(sposSecureContracts.count(contractTxId))
        sposSecureContracts.erase(contractTxId);

    if(sposOwnerContracts.count(contractTxId))
        sposOwnerContracts.erase(contractTxId);
 ...
 * https://ocidetalk.org/?topic=54527, or
 * https://github.com/ocide/ocide/pull/1393.
 */
