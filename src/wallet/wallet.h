...
public:
     ...
     * on this ocide node, and set to 0 for transactions that were created
...
private:
    ...
    std::vector<CWalletTx> sposContractsTxLoadedFromDB;
    ...
    bool AddToWalletIfSPoSContract(const CTransactionRef &tx);
    ...
    bool IsSPoSContractSpent(COutPoint outpoint) const;
...
public:
    ...
    std::map<uint256, SPoSContract> sposOwnerContracts;
    std::map<uint256, SPoSContract> sposSecureContracts;
    ...
    bool LoadSPoSContract(const CWalletTx &walletTx);
    void LoadSPoSContractFromDB(CWalletTx walletTx);
    bool RemoveSPoSContract(const uint256 &contractTxId);
    ...
    bool CreateCoinStake(unsigned int nBits, CAmount blockReward,
                         ...
                         const SPoSContract &sposContract, std::vector<const CWalletTx *> &vwtxPrev,
