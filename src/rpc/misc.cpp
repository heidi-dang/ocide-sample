...
static UniValue validateaddress(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "\nReturn information about the given ocide address.\n"
            "DEPRECATION WARNING: Parts of this command have been deprecated and moved to getaddressinfo. Clients must\n"
            "transition to using getaddressinfo to access this information before upgrading to v0.18. The following deprecated\n"
            "fields have moved to getaddressinfo and will only be shown here with -deprecatedrpc=validateaddress: ismine, iswatchonly,\n"
            "script, hex, pubkeys, sigsrequired, pubkey, addresses, embedded, iscompressed, account, timestamp, hdkeypath, kdmasterkeyid.\n"
            "\nArguments:\n"
            "1. \"address\"                    (string, required) The ocide address to validate\n"
            "\nResult:\n"
            "{\n"
...
static UniValue verifymessage(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 3)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. \"address\"         (string, required) The ocide address to use for the signature.\n"
...
static UniValue echo(const JSONRPCRequest& request)
{
    if (request.fHelp)
        throw std::runtime_error(
            ...
            "ocide-cli and the GUI. There is no server-side difference."
        );
...
static UniValue getinfo_deprecated(const JSONRPCRequest& request)
{
    throw JSONRPCError(RPC_METHOD_NOT_FOUND,
        ...
        "\nocide-cli has the option -getinfo to collect and format these in the old format."
    );
}
...
UniValue getstakingstatus(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 0)
        throw std::runtime_error(
            ...
            "  \"staking spos txid\" ,             (string)  if the wallet is sposing or not\n"
            "}\n"
            "\nExamples:\n" +
    ...
    obj.push_back(Pair("securesync", securenodeSync.IsSynced()));
    ...
    bool isSPoS = false;
    uint256 txId;
    std::tie(isSPoS, txId) = GetSPoSMinningParams();

    std::string sposStatus = "none";
    if(isSPoS)
    {
        auto helper = [&txId, &sposStatus] {
            SPoSContract contract;
            if(!SPoSUtils::CheckContract(txId, contract, true, true))
            {
                sposStatus = "Failed to find spos contract, probably spent";
                return false;
            }

            CSecurenode secureNode;
            securenodeman.Get(activeSecurenode.pubKeySecurenode, secureNode);

            if(!securenodeman.Get(activeSecurenode.pubKeySecurenode, secureNode))
            {
                sposStatus = "Securenode is not available in the list";
                return false;
            }

            if(!secureNode.IsValidForPayment())
            {
                sposStatus = "Securenode is not valid for payment";
                return false;
            }

            auto securenodePayee = CBitcoinAddress(activeSecurenode.pubKeySecurenode.GetID());

            if(contract.secureAddress != securenodePayee)
            {
                sposStatus = "Securenode is not configured for contract: " + txId.ToString();
            }
    ...
    obj.push_back(Pair("staking spos txid", isSPoS ? txId.ToString() : sposStatus));
