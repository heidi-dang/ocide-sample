...
public:
    ...
    std::unique_ptr<PendingWalletTx> createSPoSContractTransaction(CTxDestination spos_address,
            CTxDestination secure_address,
            int secure_commission,
            std::string& fail_reason) override
    {
        LOCK2(cs_main, m_wallet.cs_wallet);
        auto pending = MakeUnique<PendingWalletTxImpl>(m_wallet);
        if(!SPoSUtils::CreateSPoSTransaction(&m_wallet, pending->m_tx, pending->m_key, spos_address, secure_address, secure_commission, fail_reason)) {
            return {};
        }
        return std::move(pending);
    }
    std::unique_ptr<PendingWalletTx> createCancelContractTransaction(const SPoSContract &contract,
            std::string& fail_reason) override
    {
        LOCK2(cs_main, m_wallet.cs_wallet);
        auto pending = MakeUnique<PendingWalletTxImpl>(m_wallet);
        if(!SPoSUtils::CreateCancelContractTransaction(&m_wallet, pending->m_tx, pending->m_key, contract, fail_reason)) {
            return {};
        }
        return std::move(pending);
    }
    ...
    const std::map<uint256, SPoSContract> &getOwnerContracts() const override
    {
        return m_wallet.sposOwnerContracts;
    }
    ...
    bool getSPoSPayments(const CTransactionRef &tx, CAmount &stakeAmount, CAmount &commissionAmount,
                                     CTxDestination &sposAddress, CTxDestination &secureAddress) override
    {
        LOCK2(::cs_main, m_wallet.cs_wallet);
        return SPoSUtils::GetSPoSPayments(&m_wallet, tx, stakeAmount, commissionAmount, sposAddress, secureAddress);
    }
