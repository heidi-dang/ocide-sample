...
#include <spos/sposutils.h>            // For SPoSContract
...
public:
    ...
    //! Create spos contract transaction.
    virtual std::unique_ptr<PendingWalletTx> createSPoSContractTransaction(CTxDestination spos_address,
        CTxDestination secure_address,
        int secure_commission,
        std::string& fail_reason) = 0;

    //! Create cancel spos contract transaction.
    virtual std::unique_ptr<PendingWalletTx> createCancelContractTransaction(const SPoSContract &contract,
        std::string& fail_reason) = 0;
    ...
    virtual const std::map<uint256, SPoSContract> &getOwnerContracts() const = 0;
    ...
    virtual bool getSPoSPayments(const CTransactionRef &tx, CAmount &stakeAmount, CAmount &commissionAmount,
                                 CTxDestination &sposAddress, CTxDestination &secureAddress) = 0;
