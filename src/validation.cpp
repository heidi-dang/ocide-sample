...
#if defined(NDEBUG)
# error "OCIDE cannot be compiled without assertions."
#endif
...
static bool WriteBlockToDisk(const CBlock& block, CDiskBlockPos& pos, const CMessageHeader::MessageStartChars& messageStart)
{
    ...
    // Write block
    long fileOusPos = ftell(fileout.Get());
    if (fileOusPos < 0)
        return error("WriteBlockToDisk: ftell failed");
    pos.nPos = (unsigned int)fileOusPos;
    fileout << block;
...
CAmount GetBlockSubsidy(int nPrevHeight, const Consensus::Params& consensusParams, bool fSuperblockPartOnly)
{
    ...
    if (nPrevHeight >= 1 && nPrevHeight < 200) {
        nSubsidy = 1.25 * COIN;
    } else if (nPrevHeight >= 200 && nPrevHeight < 400) {
        nSubsidy = 2.5 * COIN;
    } else if (nPrevHeight >= 400 && nPrevHeight < 600) {
        nSubsidy = 5 * COIN;
    } else if (nPrevHeight >= 600 && nPrevHeight < 800) {
        nSubsidy = 10 * COIN;
    }
...
int32_t ComputeBlockVersion(const CBlockIndex* pindexPrev, const Consensus::Params& params)
{
    ...
    for (int i = 0; i < (int)Consensus::MAX_VERSION_BITS_DEPLOYMENTS; i++) {
        ThresholdState state = VersionBitsState(pindexPrev, params, static_cast<Consensus::DeploymensPos>(i), versionbitscache);
        if (state == ThresholdState::LOCKED_IN || state == ThresholdState::STARTED) {
            nVersion |= VersionBitsMask(params, static_cast<Consensus::DeploymensPos>(i));
        }
    }
                               ...
                               CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    // OCIDE : MODIFIED TO CHECK MASTERNODE PAYMENTS AND SUPERBLOCKS
    ...
    if (!IsBlockValueValid(block, pindex->nHeight, expectedReward, pindex->nMint, strError)) {
        return state.DoS(0, error("ConnectBlock(OCIDE): %s", strError), REJECT_INVALID, "bad-cb-amount");
    }

    const auto& coinbaseTransaction = (pindex->nHeight > Params().GetConsensus().nLastPoWBlock ? block.vtx[1] : block.vtx[0]);

    if(block.IsSPoSBlock() && !SPoSUtils::IsSecurePaymentValid(state, block, pindex->nHeight, expectedReward, pindex->nMint)) {
        return false;
    }

    if (!IsBlockPayeeValid(coinbaseTransaction, pindex->nHeight, expectedReward, pindex->nMint)) {
        //        mapRejectedBlocks.insert(std::make_pair(block.GetHash(), GetTime()));
        return state.DoS(0, error("ConnectBlock(OCIDE): couldn't find masternode or superblock payments"),
...
bool CheckBlock(const CBlock& block, CValidationState& state, const Consensus::Params& consensusParams, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckContractOutpoint)
{
    ...
    if (block.IsProofOfStake()) {
        ...
        if(block.IsSPoSBlock()) {
            bool fCheckSPoSSignature = block.GetBlockTime() >
                    Params().GetConsensus().nSPoSContractSignatureDeploymentTime;

            if(!SPoSUtils::CheckContract(block.hashSPoSContractTx, contract, fCheckSPoSSignature, fCheckContractOutpoint)) {
                state.DoS(100, error("CheckBlock(): check contract failed for spos block %s\n", hash.ToString().c_str()));
                return false;
            }

            block.txSPoSContract = contract.rawTx;
        }
...
{
    int commispos = GetWitnessCommitmentIndex(block);
    static const std::vector<unsigned char> nonce(32, 0x00);
    if (commispos != -1 && IsWitnessEnabled(pindexPrev, consensusParams) && !block.vtx[0]->HasWitness()) {
 ...
 */
static uint256 WitnessComittmentForPoSBlock(const CBlock &block, int commispos, bool &malleated)
{   
//    if(!block.IsProofOfStake())
...
//    vout.erase(std::begin(vout) + commispos);
...
static bool ContextualCheckBlock(const CBlock& block, CValidationState& state, const Consensus::Params& consensusParams, const CBlockIndex* pindexPrev)
{
    ...
    if (VersionBitsState(pindexPrev, consensusParams, Consensus::DEPLOYMENT_SEGWIT, versionbitscache) == ThresholdState::ACTIVE) {
        ...
        if (commispos != -1) {
            ...
            uint256 hashWitness = WitnessComittmentForPoSBlock(block, commispos, malleated);
            ...
            if (memcmp(hashWitness.begin(), &block.vtx[0]->vout[commispos].scriptPubKey[6], 32)) {
...
bool LoadExternalBlockFile(const CChainParams& chainparams, FILE* fileIn, CDiskBlockPos *dbp)
{
    ...
    try {
        ...
        uint64_t nRewind = blkdat.GesPos();
        while (!blkdat.eof()) {
            boost::this_thread::interruption_point();

            blkdat.SesPos(nRewind);
            ...
            try {
                // read block
                uint64_t nBlockPos = blkdat.GesPos();
                if (dbp)
                    dbp->nPos = nBlockPos;
                blkdat.SetLimit(nBlockPos + nSize);
                blkdat.SesPos(nBlockPos);
                std::shared_ptr<CBlock> pblock = std::make_shared<CBlock>();
                CBlock& block = *pblock;
                blkdat >> block;
                nRewind = blkdat.GesPos();
...
ThresholdState VersionBitsTipState(const Consensus::Params& params, Consensus::DeploymensPos pos)
{
    LOCK(cs_main);
    return VersionBitsState(chainActive.Tip(), params, pos, versionbitscache);
}

BIP9Stats VersionBitsTipStatistics(const Consensus::Params& params, Consensus::DeploymensPos pos)
{
    LOCK(cs_main);
    return VersionBitsStatistics(chainActive.Tip(), params, pos);
}

int VersionBitsTipStateSinceHeight(const Consensus::Params& params, Consensus::DeploymensPos pos)
{
    LOCK(cs_main);
