...
enum {
    ...
    MSG_SECURENODE_VERIFY,
    MSG_SECURENODE_ANNOUNCE,
    MSG_SECURENODE_PING
};
}
...
static const MapSporkHandlers &GetMapGetDataHandlers()
{
    ...
    if(sporkHandlers.empty())
    {
        ...
        ADD_HANDLER(MSG_SECURENODE_ANNOUNCE, {
                        if(securenodeman.mapSeenSecurenodeBroadcast.count(hash)){
                            return msgMaker.Make(NetMsgType::SECURENODEANNOUNCE, securenodeman.mapSeenSecurenodeBroadcast[hash].second);
                        }
                        return {};
                    });
        ...
        ADD_HANDLER(MSG_SECURENODE_PING, {
                        if(securenodeman.mapSeenSecurenodePing.count(hash)) {
                            return msgMaker.Make(NetMsgType::SECURENODEPING, securenodeman.mapSeenSecurenodePing[hash]);
                        }
                        return {};
                    });
        ...
        ADD_HANDLER(MSG_SECURENODE_VERIFY, {
                        if(securenodeman.mapSeenSecurenodeVerification.count(hash)) {
                            return msgMaker.Make(NetMsgType::SECURENODEVERIFY, securenodeman.mapSeenSecurenodeVerification[hash]);
                        }
                        return {};
                    });
    }
...
bool net_processing_ocide::ProcessGetData(CNode *pfrom, const Consensus::Params &consensusParams, CConnman *connman, const CInv &inv)
{
...
void net_processing_ocide::ProcessExtension(CNode *pfrom, const std::string &strCommand, CDataStream &vRecv, CConnman *connman)
{
    mnodeman.ProcessMessage(pfrom, strCommand, vRecv, *connman);
    mnpayments.ProcessMessage(pfrom, strCommand, vRecv, *connman);
    securenodeman.ProcessMessage(pfrom, strCommand, vRecv, *connman);
    instantsend.ProcessMessage(pfrom, strCommand, vRecv, *connman);
    sporkManager.ProcessSpork(pfrom, strCommand, vRecv, connman);
    masternodeSync.ProcessMessage(pfrom, strCommand, vRecv);
    securenodeSync.ProcessMessage(pfrom, strCommand, vRecv);
    governance.ProcessMessage(pfrom, strCommand, vRecv, *connman);
}

void net_processing_ocide::ThreadProcessExtensions(CConnman *pConnman)
{
    ...
    while (!ShutdownRequested())
    {
        ...
        if(!ShutdownRequested()) {
            ...
            if(securenodeSync.IsBlockchainSynced()) {
...
bool net_processing_ocide::AlreadyHave(const CInv &inv)
{
    switch(inv.type)
    {
    /*
    OCIDE Related Inventory Messages
    ...
    */
    ...
    case MSG_SECURENODE_ANNOUNCE:
        return securenodeman.mapSeenSecurenodeBroadcast.count(inv.hash) && !securenodeman.IsMnbRecoveryRequested(inv.hash);

    case MSG_MASTERNODE_PING:
        return mnodeman.mapSeenMasternodePing.count(inv.hash);
    case MSG_SECURENODE_PING:
        return securenodeman.mapSeenSecurenodePing.count(inv.hash);
    ...
    case MSG_SECURENODE_VERIFY:
        return securenodeman.mapSeenSecurenodeVerification.count(inv.hash);
    }
    return true;
}
...
static int MapLegacyToCurrent(int nLegacyType)
{
    ...
    case LegacyInvMsg::MSG_SECURENODE_VERIFY: return MSG_SECURENODE_VERIFY;
    case LegacyInvMsg::MSG_SECURENODE_ANNOUNCE: return MSG_SECURENODE_ANNOUNCE;
    case LegacyInvMsg::MSG_SECURENODE_PING: return MSG_SECURENODE_PING;
    }
...
static int MapCurrentToLegacy(int nCurrentType)
{
    ...
    case MSG_SECURENODE_VERIFY: return LegacyInvMsg::MSG_SECURENODE_VERIFY;
    case MSG_SECURENODE_ANNOUNCE: return LegacyInvMsg::MSG_SECURENODE_ANNOUNCE;
    case MSG_SECURENODE_PING: return LegacyInvMsg::MSG_SECURENODE_PING;
    }

    return nCurrentType;
}

bool net_processing_ocide::TransformInvForLegacyVersion(CInv &inv, CNode *pfrom, bool fForSending)
{
